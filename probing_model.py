...
class ProbingClassifier(models.Model):
    def __init__(self,
        ...
        # TODO(students): start
        # Single Feed Forward Layer with No activation
        self.classification_layer = layers.Dense(classes_num, use_bias=True, activation=None)
        # TODO(students): end
    ...
    def call(self, inputs: tf.Tensor, training: bool = False) -> tf.Tensor:
        ...
        """
        # TODO(students): start
        results = self._pretrained_model(inputs, training=False)
        layer_representations = results["layer_representations"]
        logits = self.classification_layer(layer_representations[:, self._layer_num - 1, :])
        # TODO(students): end
